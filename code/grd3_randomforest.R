...
title("Rec. at least A's and B's mtry: Blind, 20% sample")

# Estimates the race blind random forest. Based on sampsize tuning, looks like 1500 trees will suffice.
# Optimal mtry = 40
set.seed(5000)
RF_blind = randomForest(blind_covar[train, ], y[train], ntree = 1500, sampsize = 0.2*trg_sz, mtry = 40)
...
title("Rec. at least A's and B's sampsize: Aware")
legend("topright", c("Default", "20%", "40%", "60%", "80%"), 
       lty = c(1, 1, 1, 1, 1), lwd = c(1, 1, 1, 1, 1), 
       col = c("blue", "red", "green", "orange", "cyan"))
...
plot(raceaware_rf[, 1], raceaware_rf[, 2], type = "o", col = "black",
     xlab = "Variables rand. sampled for split", ylab = "OOB Error")
title("Rec. at least A's and B's mtry: Aware, default sample")

# Estimates the race aware random forest
# Based on sampsize tuning, looks like 1500 trees will suffice.
# Set mtry to 20
set.seed(5000)
RF_aware = randomForest(race_covar[train, ], y[train], ntree = 1500, mtry = 20)
...
ggplot(heatmap.melted, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  ggtitle("Rec. at least A's and B's: Deciles of pred. prob. blind vs. aware") +   
...
ggplot(heatmap.melted.w, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ylab('Race aware pred. prob') + 
  xlab('Race blind pred. prob') + 
  ggtitle("Rec. at least A's and B's: Deciles of pred. prob. blind vs. aware, whites") + 
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red")
...
ggplot(heatmap.melted.b, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red") + 
  ggtitle("Rec. at least A's and B's: Deciles of pred. prob. blind vs. aware, blacks") 
...
plot(blind_bin_avg$Group.1, blind_bin_avg$y.tst, type = 'p', col = 'red', 
     xlab = "Deciles of pred. prob. of rec. mostly A's and B's", 
     ylab = "Prob. of rec. mostly A's and B's", ylim = c(0, 1), xaxt = 'n')
lines(blind_bin_avg$Group.1, blind_bin_avg$raceblind_prob, type = 'l', col = 'black')
title(main = "Rec. mostly A's and B's calib. curve: Blind predictor")
legend(8, 0.1, c("observed", "fitted"), 
...
title(main = "Rec. mostly A's and B's calib. curve: Blind, by group")
legend(1, 0.9, c('fit. whites', 'fit. blacks', 'obs. whites', 'obs. blacks'), 
...
title(main = "Rec. mostly A's and B's calib. curve: Aware predictor")
legend(8, 0.1, c("observed", "fitted"), 
       lty = c(NA, 1), lwd = c(NA, 2), pch = c(1, NA), col = c("red", "black"))
axis(side = 1, c(1:10))
...
plot(wht_bin_avg$Group.1, wht_bin_avg$raceaware_prob, type = 'l', lty = 1, col = 'black', 
     xlab = "Deciles of pred. prob. of rec. mostly A's and B's",
     ylab = "Prob. of rec. mostly A's and B's", ylim = c(0, 1))
...
title(main = "Rec. mostly A's and B's calib. curve: Aware, by group")
legend(2, 0.9, c('fit. whites', 'fit. blacks', 'obs. whites', 'obs. blacks'), 
       lty = c(1, 2, NA, NA), lwd = c(2, 2, NA, NA), pch = c(NA, NA, 0, 1), 
       col = c('black', 'black', 'blue', 'red'))
axis(side = 1, c(1:10))
...
plot(whites$x, whites$y, type = 'l', lty = 1, col = 'red', 
     xlab = "Change in pred. prob. of rec. mostly A's and B's", 
     ylab = "Density", xlim = c(-0.25, 0.25))
...
partialPlot(RF_aware, race_covar[race_covar$black == 1], F12XRSTD,
            which.class = 1, col = 'red', ylim = c(-1, 0), xlim = c(0, 68),
            ylab = "Pred. prob.",
...
legend(-30, -1.5, inset = c(0, 0), xpd = NA, legend =  c("blacks", "whites"), horiz = TRUE, 
...
partialPlot(RF_aware, race_covar[race_covar$black == 1], F22XRSTD,
            which.class = 1, col = 'red', ylim = c(-1, 0), xlim = c(0, 66),
            ylab = "Pred. prob.",
            main = "English") # english scores
...
partialPlot(RF_aware, race_covar[race_covar$black == 1], F2RHENG2,
            which.class = 1, col = 'red', ylim = c(-1, 0), xlim = c(2, 13),
            ylab = "Pred. prob.",
...
legend(-5, -1.5, inset = c(0, 0), xpd = NA, legend =  c("blacks", "whites"), horiz = TRUE, 
       lty = c(1, 1), lwd = c(2, 2), col = c('red', 'blue'), cex = 1, bty = 'n')
...
title("Rec. at least A's and B's sampsize: orthog")
legend("topright", c("Default", "20%", "40%", "60%", "80%"), 
       lty = c(1, 1, 1, 1, 1), lwd = c(1, 1, 1, 1, 1), 
       col = c("blue", "red", "green", "orange", "cyan"))
...
# Best forest is returned and used from this
set.seed(4000)
raceorthog_rf = tuneRF(orthog_covar[train, ], y[train], ntreeTry = 2000, sampesize = 0.2*trg_sz, stepfactor = 1.5,
                      improve = 1e-5, trace = TRUE, plot = TRUE, doBest = FALSE)

# Plots MTRY tuning
plot(raceorthog_rf[, 1], raceorthog_rf[, 2], type = "o", col = "black",
     xlab = "Variables rand. sampled for split", ylab = "OOB Error")
title("Rec. at least A's and B's mtry: Orthog, 20% sample")

# Estimates the race blind random forest. Based on sampsize tuning, looks like 1500 trees will suffice.
# Optimal mtry = 40
set.seed(5000)
RF_orthog = randomForest(orthog_covar[train, ], y[train], ntree = 1500, sampsize = 0.2*trg_sz, mtry = 40)
...
##################
# DECISION RULE 2: Admit same composition as race blind admit top 50%
##################
# race orthog admit rule
...
blind_admit[race_covar.tst$white == 1 & raceblind_prob == whiteblind_cutoff] = c(1, 0)
blind_admit.wht = sum(race_covar.tst$white[blind_admit == 1])
blind_admit.blk = sum(race_covar.tst$black[blind_admit == 1])
...
orthog_admit[race_covar.tst$white == 1 & raceorthog_prob == whiteorthog_cutoff] = c(1, 1, 0, 0) 
orthog_admit.wht = sum(race_covar.tst$white[orthog_admit == 1])
orthog_admit.blk = sum(race_covar.tst$black[orthog_admit == 1])
