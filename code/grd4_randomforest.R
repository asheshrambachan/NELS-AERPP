...
# Make sure outcome variable in data_clean.R set to 4.
...
title("Rec. mostly B's and C's sampsize: Blind")
legend("topright", c("Default", "20%", "40%", "60%", "80%"), 
       lty = c(1, 1, 1, 1, 1), lwd = c(1, 1, 1, 1, 1), 
       col = c("blue", "red", "green", "orange", "cyan"))

# Selects sample size set to default training size with 2000 trees
# Tunes number of variables randomly sampled at each split
# Best forest is returned and used from this
set.seed(4000)
raceblind_rf = tuneRF(blind_covar[train, ], y[train], ntreeTry = 2000, stepfactor = 1.5,
...
title("Rec. mostly B's and C's mtry tuning: Blind, Def.")

# Estimates the race blind random forest. Based on sampsize tuning, looks like 1500 trees will suffice.
# Optimal mtry = 10
set.seed(5000)
RF_blind = randomForest(blind_covar[train, ], y[train], ntree = 1500, mtry = 10)
...
# Tunes sample size
set.seed(3000)
raceaware_tune = randomForest(race_covar[train, ], y[train], ntree = 2000, importance = FALSE)
...
title("Rec. mostly B's and C's sampsize: Aware")
legend("topright", c("Default", "20%", "40%", "60%", "80%"), 
       lty = c(1, 1, 1, 1, 1), lwd = c(1, 1, 1, 1, 1), 
       col = c("blue", "red", "green", "orange", "cyan"))

# Select default Number of trees set to search over 2000. 
# tunes number of variables randomly sampled at each split
set.seed(4000)
raceaware_rf = tuneRF(race_covar[train, ], y[train], ntreeTry = 2000, stepfactor = 1.5, 
                      improve = 1e-5, trace = TRUE, plot = TRUE, doBest = FALSE)

# Plots MTRY tuning
plot(raceaware_rf[, 1], raceaware_rf[, 2], type = "o", col = "black",
     xlab = "Variables rand. sampled for split", ylab = "OOB Error")
title("Rec. mostly B's and C's mtry: Aware, default")

# Estimates the race aware random forest
# Based on sampsize tuning, looks like 1500 trees will suffice.
set.seed(5000)
RF_aware = randomForest(race_covar[train, ], y[train], ntree = 1500)
...
######################################################## 
# Considers effects of policy rule based on race-blind #
# and race-aware decision rules                        #
########################################################
# full sample
...
ggplot(heatmap.melted, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  ggtitle("Rec. mostly B's and C's: Deciles of pred. prob. blind vs. aware") +   
...
ggplot(heatmap.melted.w, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ylab('Race aware pred. prob') + 
  xlab('Race blind pred. prob') + 
  ggtitle("Rec. mostly B's and C's: Deciles of pred. prob. blind vs. aware, whites") + 
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red")
...
ggplot(heatmap.melted.b, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red") + 
  ggtitle("Rec. mostly B's and C's: Deciles of pred. prob. blind vs. aware, blacks") 
...
plot(blind_bin_avg$Group.1, blind_bin_avg$y.tst, type = 'p', col = 'red', 
     xlab = "Deciles of pred. prob. of rec. mostly B's and C's", 
     ylab = "Prob. of rec. mostly B's and C's", ylim = c(0.6, 1), xaxt = 'n')
lines(blind_bin_avg$Group.1, blind_bin_avg$raceblind_prob, type = 'l', col = 'black')
title(main = "Rec. mostly B's and C's calib. curve: Blind predictor")
legend(8, 0.7, c("observed", "fitted"), 
...
title(main = "Rec. mostly B's and C's calib. curve: Blind, by group")
legend(8, 0.7, c('fit. whites', 'fit. blacks', 'obs. whites', 'obs. blacks'), 
...
title(main = "Rec. mostly B's and C's calib. curve: Aware predictor")
legend(8, 0.7, c("observed", "fitted"), 
       lty = c(NA, 1), lwd = c(NA, 2), pch = c(1, NA), col = c("red", "black"))
axis(side = 1, c(1:10))
...
plot(wht_bin_avg$Group.1, wht_bin_avg$raceaware_prob, type = 'l', lty = 1, col = 'black', 
     xlab = "Deciles of pred. prob. of rec. mostly B's and C's",
     ylab = "Prob. of rec. mostly B's and C's", ylim = c(0.6, 1))
...
title(main = "Rec. mostly B's and C's calib. curve: Aware, by group")
legend(8, 0.7, c('fit. whites', 'fit. blacks', 'obs. whites', 'obs. blacks'), 
       lty = c(1, 2, NA, NA), lwd = c(2, 2, NA, NA), pch = c(NA, NA, 0, 1), 
       col = c('black', 'black', 'blue', 'red'))
axis(side = 1, c(1:10))
...
plot(whites$x, whites$y, type = 'l', lty = 1, col = 'red', 
     xlab = "Change in pred. prob. of rec. mostly B's and C's", 
     ylab = "Density", xlim = c(-0.05, 0.07))
title(main = "Difference of Yhat of blind vs. aware predictors")
lines(blacks$x, blacks$y, type = 'l', lty = 2, col = 'blue')
legend('topright', c('whites', 'blacks'), lty = c(1, 2), lwd = c(2, 2), 
       col = c('red', 'blue'))
...
partialPlot(RF_aware, race_covar[race_covar$black == 1], F12XRSTD,
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(0, 68),
            ylab = "Pred. prob.",
...
partialPlot(RF_aware, race_covar[race_covar$black == 1], F22XRSTD,
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(0, 66),
            ylab = "Pred. prob.",
            main = "English") # english scores
partialPlot(RF_aware, race_covar[race_covar$white == 1], F22XRSTD, 
            which.class = 1, add = TRUE, col = 'blue') # english scores

partialPlot(RF_aware, race_covar[race_covar$black == 1], F22XMSTD,
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(0, 66),
            main = "Math") # math scores
partialPlot(RF_aware, race_covar[race_covar$white == 1], F22XMSTD, 
            which.class = 1, add = TRUE, col = 'blue') # math scores

partialPlot(RF_aware, race_covar[race_covar$black == 1], F22XSSTD, 
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(0, 66),
            ylab = "Pred. prob.", xlab = "Std. scores", 
            main = "Science") # science scores
partialPlot(RF_aware, race_covar[race_covar$white == 1], F22XSSTD, 
            which.class = 1, col = "blue", add = TRUE) # science scores

partialPlot(RF_aware, race_covar[race_covar$black == 1], F22XHSTD, 
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(0, 66),
            xlab = "Std. scores",
            main = "History") # history scores
partialPlot(RF_aware, race_covar[race_covar$white == 1], F22XHSTD, 
            which.class = 1, col = "blue", add = TRUE) # History scores
...
partialPlot(RF_aware, race_covar[race_covar$black == 1], F2RHENG2,
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(2, 13),
            ylab = "Pred. prob.",
            main = "English") # english grades
partialPlot(RF_aware, race_covar[race_covar$white == 1], F2RHENG2, 
            which.class = 1, add = TRUE, col = 'blue') # english grades

partialPlot(RF_aware, race_covar[race_covar$black == 1], F2RHMAG2,
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(2, 13),
            main = "Math") # math grades
partialPlot(RF_aware, race_covar[race_covar$white == 1], F2RHMAG2, 
            which.class = 1, add = TRUE, col = 'blue') # math grades

partialPlot(RF_aware, race_covar[race_covar$black == 1], F2RHSCG2, 
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(2, 13),
            ylab = "Pred. prob.", xlab = "Grades", 
            main = "Science") # science grades
partialPlot(RF_aware, race_covar[race_covar$white == 1], F2RHSCG2, 
            which.class = 1, col = "blue", add = TRUE) # science grades

partialPlot(RF_aware, race_covar[race_covar$black == 1], F2RHSOG2, 
            which.class = 1, col = 'red', ylim = c(0, 1.75), xlim = c(2, 13),
            xlab = "Grades",
            main = "Social Studies") # social studies grades
partialPlot(RF_aware, race_covar[race_covar$white == 1], F2RHSOG2, 
            which.class = 1, col = "blue", add = TRUE) # social studies grades
title(main = "Partial dep. of F2 grades: Race Aware", outer = TRUE, cex = 1.5)
...
# loop through each variable, regress of black indicator
# store residuals
...
title("Rec. mostly B's and C's sampsize: Orthog")
legend("topright", c("Default", "20%", "40%", "60%", "80%"), 
       lty = c(1, 1, 1, 1, 1), lwd = c(1, 1, 1, 1, 1), 
       col = c("blue", "red", "green", "orange", "cyan"))

# Selects sample size of default of training size
# Tunes number of variables randomly sampled at each split
# Best forest is returned and used from this
set.seed(4000)
raceorthog_rf = tuneRF(orthog_covar[train, ], y[train], ntreeTry = 2000, stepfactor = 1.5,
                       improve = 1e-5, trace = TRUE, plot = TRUE, doBest = FALSE)

# Plots MTRY tuning
plot(raceorthog_rf[, 1], raceorthog_rf[, 2], type = "o", col = "black",
     xlab = "Variables rand. sampled for split", ylab = "OOB Error")
title("Rec. at least B's and C's mtry: Orthog, 20% sample")

# Estimates the race blind random forest. Based on sampsize tuning, looks like 1500 trees will suffice.
# Optimal mtry = 10
set.seed(5000)
RF_orthog = randomForest(orthog_covar[train, ], y[train], ntree = 1500, mtry = 10)
...
orthog_admit[race_covar.tst$white == 1 & raceorthog_prob == whiteorthog_cutoff] = c(1, 0, 0) 
orthog_admit.wht = sum(race_covar.tst$white[orthog_admit == 1])
orthog_admit.blk = sum(race_covar.tst$black[orthog_admit == 1])
...
aware_admit[race_covar.tst$white == 1 & raceaware_prob == whiteaware_cutoff] = c(1, 1, 0, 0, 0)
aware_admit.wht = sum(race_covar.tst$white[aware_admit == 1])
aware_admit.blk = sum(race_covar.tst$black[aware_admit == 1])
...
blind_admit[race_covar.tst$white == 1 & raceblind_prob == whiteblind_cutoff] = c(1, 1, 0, 0)
blind_admit.wht = sum(race_covar.tst$white[blind_admit == 1])
blind_admit.blk = sum(race_covar.tst$black[blind_admit == 1])
...
orthog_admit[race_covar.tst$black == 1 & raceorthog_prob == blackorthog_cutoff] = c(1, 1, 0) 
orthog_admit.wht = sum(race_covar.tst$white[orthog_admit == 1])
orthog_admit.blk = sum(race_covar.tst$black[orthog_admit == 1])

print(paste0("Aware admit rule admits ", sum(orthog_admit)))
print(paste0("Aware admit rule admits ", orthog_admit.wht, " whites"))
print(paste0("Aware admit rule admits ", orthog_admit.blk, " blacks"))
...
aware_admit = ifelse(race_covar.tst$white == 1 & raceaware_prob >= whiteaware_cutoff |
                       race_covar.tst$black == 1 & raceaware_prob >= blackaware_cutoff, 1, 0)
aware_admit[race_covar.tst$white == 1 & raceaware_prob == whiteaware_cutoff] = c(1, 1, 1, 0, 0, 0)
aware_admit.wht = sum(race_covar.tst$white[aware_admit == 1])
aware_admit.blk = sum(race_covar.tst$black[aware_admit == 1])
...
orthog_admit = ifelse(race_covar.tst$white == 1 & raceorthog_prob >= whiteorthog_cutoff |
                        race_covar.tst$black == 1 & raceorthog_prob >= blackorthog_cutoff, 1, 0)
# exists tie for last-in whites. Take first index
orthog_admit[race_covar.tst$white == 1 & raceorthog_prob == whiteorthog_cutoff] = c(1, 1, 0)
orthog_admit.wht = sum(race_covar.tst$white[orthog_admit == 1])
orthog_admit.blk = sum(race_covar.tst$black[orthog_admit == 1])
...
aware_admit = ifelse(race_covar.tst$white == 1 & raceaware_prob >= whiteaware_cutoff |
                       race_covar.tst$black == 1 & raceaware_prob >= blackaware_cutoff, 1, 0)
aware_admit[race_covar.tst$white == 1 & raceaware_prob == whiteaware_cutoff] = c(1, 0)
aware_admit.wht = sum(race_covar.tst$white[aware_admit == 1])
aware_admit.blk = sum(race_covar.tst$black[aware_admit == 1])
