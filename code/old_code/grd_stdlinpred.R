...
# Description: This computes a simple linear predictor of undergrad 
# grades on average of z-scores of standardized test scores in
...
# Constructs dataset 
tmp_data = data.frame(NELS$grd_out, compstd, black, white)

# Raceblind predictor
raceblind_lm = lm(NELS.grd_out ~ 1 + compstd, data = tmp_data, subset = (black == 1 | white == 1))
blind_pred = raceblind_lm$coefficients[1] + raceblind_lm$coefficients[2]*tmp_data$compstd; # race-blind predicted values

# Constructs calibration curve for race-blind predictor
blind_data = data.frame(NELS$grd_out, blind_pred, black, white)
blind_data$pctile = ntile(blind_data$blind_pred, 10) # constructs deciles of fitted values
...
plot(blind_bin_avg$Group.1, blind_bin_avg$NELS.grd_out, type = 'p', col = 'red', 
     xlab = "Quintiles of predicted prob. of rec. at least 'mostly Bs'", 
     ylab = "Prob. of rec. at least 'mostly Bs'", xaxt = "n", 
     ylim = c(0.6, 1))
lines(blind_bin_avg$Group.1, blind_bin_avg$blind_pred, type = 'l', col = 'black')
title(main = "Grades calib. curve: Race blind lin. pred.")
legend(7, 0.65, c("Observed", "Fitted"), 
...
plot(blind_bin_avg$Group.1, blind_bin_avg$blind_pred, type = "l", col = "black",
     ...
     xaxt = "n", ylim = c(0.6, 1))
lines(white_bin_avg$Group.1, white_bin_avg$NELS.grd_out, type = 'p', pch = 0, col = 'blue')
lines(black_bin_avg$Group.1, black_bin_avg$NELS.grd_out, type = 'p', col = 'red')
title("Grades calib. curve: By racial group")
legend(7, 0.7, c("Predicted", "White", "Black"), 
       lty = c(1, NA, NA), lwd = c(2, NA, NA), pch = c(NA, 0, 1), 
       col = c("black", "blue", "red"))
axis(side = 1, at = c(1:10))

# Race aware predictors
black_lm = lm(NELS.grd_out ~ 1 + compstd, data = tmp_data, subset = (black == 1))
white_lm = lm(NELS.grd_out ~ 1 + compstd, data = tmp_data, subset = (white == 1))
...
plot(compstd_rng, blind_pred, type = "l", col = "red", xlab = "Composite Std. Test Z-Scores", 
     ylab = "Fitted prob. of receiving at least mostly B's")
