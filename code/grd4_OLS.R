...
blind_ols.prob = predict(blind_ols, blind_covar[-train, ])
aware_ols.prob = predict(aware_ols, race_covar[-train, ])
orthog_ols.prob = predict(orthog_ols, orthog_covar[-train, ])
...
ggplot(heatmap.melted, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  ggtitle("Rec. at least mostly B's and C's: Pred. prob. blind vs. aware") +   
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red")

# whites only
wht.tst = tst_sz - sum(race_covar.tst$black)
...
ggplot(heatmap.melted.w, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ylab('Race aware, deciles of pred. prob') + 
  xlab('Race blind, deciles of pred. prob') + 
  ggtitle("Rec. at least mostly B's and C's: Pred. prob. blind vs. aware, whites") + 
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red")
...
ggplot(heatmap.melted.b, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red") + 
  ggtitle("Rec. at least mostly B's and C's: Pred. prob. blind vs. aware, blacks") 
...
ggplot(data, aes(coefficient, covariates)) + geom_point(aes(color = race)) + 
  geom_line(aes(group = covariates)) + ggtitle("Rec. at least mostly B's and C's: Coef. on std. test scores")
...
data = data.frame(covariates, race, coefficient)
ggplot(data, aes(coefficient, covariates)) + geom_point(aes(color = race)) +
  geom_line(aes(group = covariates)) + ggtitle("Rec. at least mostly A's and B's: Coef. on grades and credits")
...
plot(xrg, mstd.blk, type = 'l', col = 'blue', ylim = c(-0.5, 1), 
     xlab = "", ylab = "", main = 'Math')
lines(xrg, mstd.wht, type = 'l', col = 'red')

plot(xrg, sstd.blk, type = 'l', col = 'blue', ylim = c(-0.3, 1), 
     ylab = 'Change in pred. prob', xlab = 'Std. scores', main = 'Science')
lines(xrg, sstd.wht, type = 'l', col = 'red')

plot(xrg, hstd.blk, type = 'l', col = 'blue', ylim = c(-1, 0.2), 
...
legend(-50, -1.75, xpd = NA, legend = c('blacks', 'whites'), horiz = TRUE,
