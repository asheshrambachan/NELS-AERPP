...
title("Bachelor rec. sampsize tuning: Race blind")
...
# 80% appears to perform the best
set.seed(4000)
raceblind_rf = tuneRF(blind_covar[train, ], y[train], ntreeTry = 2000, sampsize = 0.8*trg_sz, stepfactor = 1.5,
                        improve = 1e-5, trace = TRUE, plot = FALSE, doBest = FALSE)

plot(raceblind_rf[,1], raceblind_rf[, 2], type = "o", col = "black",
     xlab = "Variables rand. sampled for split", ylab = "OOB Error" )
title("Bachelor rec. mtry: Blind, 80% sample size")

# Trains the race blind random forest
# Default sample size appears to perform the best
# Looks like 1500 trees will suffice
set.seed(5000)
RF_blind = randomForest(blind_covar[train, ], y[train], ntree = 1500, mtry = 80, sampsize = 0.8*trg_sz)

######################################
# Construct Race-Aware Random Forest #
######################################
set.seed(3000)
raceaware_tune = randomForest(race_covar[train, ], y[train], ntree = 2000)
...
title("Bachelor rec. sampsize tuning: Race aware")
legend("topright", c("Default", "20%", "40%", "60%", "80%"), 
       lty = c(1, 1, 1, 1, 1), lwd = c(1, 1, 1, 1, 1), 
       col = c("blue", "red", "green", "orange", "cyan"))

# Tunes number of variables randomly sampled at each split
# Sample size of 80% the training set performs the best
set.seed(4000)
raceaware_rf = tuneRF(race_covar[train, ], y[train], sampsize = 0.8*trg_sz, ntreeTry = 2000, stepfactor = 1.5, 
                      improve = 1e-5, trace = TRUE, plot = FALSE, doBest = FALSE)

plot(raceaware_rf[, 1], raceaware_rf[, 2], xlab = "Variables rand. sampled for split", 
     ylab = "OOB Error", type = "o", col = "black")
title("Bachelor rec. mtry tuning: Aware, 80% sample size")

# Estimates race aware random forest
# sample size set at 80% and mtry set to 20,
set.seed(5000)
RF_aware = randomForest(race_covar[train, ], y[train], sampsize = 0.8*trg_sz, 
                        mtry = 20, ntree = 1500)
...
ggplot(heatmap.melted, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  ggtitle("Rec. bachelor's: Deciles of pred. prob. blind vs. aware") +   
...
ggplot(heatmap.melted.w, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ylab('Race aware pred. prob') + 
  xlab('Race blind pred. prob') + 
  ggtitle("Rec. bachelor's: Deciles of pred. prob. blind vs. aware, whites") + 
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red")
...
ggplot(heatmap.melted.b, aes(x = Var1, y = Var2, fill = value)) + geom_tile(aes(fill=value, col=value)) + 
  ...
  scale_colour_gradient(low = "white", high = "red") + 
  scale_fill_gradient(low = "white", high = "red") + 
  ggtitle("Rec. bachelor's: Deciles of pred. prob. blind vs. aware, blacks") 
...
plot(blind_bin_avg$Group.1, blind_bin_avg$y.tst, type = 'p', col = 'red', 
     xlab = "Deciles of pred. prob. of rec. bachelor's", 
     ylab = "Prob. of rec. bachelor's", ylim = c(0, 1), xaxt = 'n')
lines(blind_bin_avg$Group.1, blind_bin_avg$raceblind_prob, type = 'l', col = 'black')
title(main = "Rec. bachelor's calib. curve: Blind predictor")
legend(8, 0.2, c("observed", "fitted"), 
...
title(main = "Rec. bachelor's calib. curve: Blind, by group")
legend(8, 0.2, c('fit. whites', 'fit. blacks', 'obs. whites', 'obs. blacks'), 
...
title(main = "Rec. bachelor's calib. curve: Aware predictor")
legend(8, 0.2, c("observed", "fitted"), 
       lty = c(NA, 1), lwd = c(NA, 2), pch = c(1, NA), col = c("red", "black"))
axis(side = 1, c(1:10))
...
plot(wht_bin_avg$Group.1, wht_bin_avg$raceaware_prob, type = 'l', lty = 1, col = 'black', 
     xlab = "Deciles of pred. prob. of rec. bachelor's",
     ylab = "Prob. of rec. bachelor's", ylim = c(0, 1))
...
title(main = "Rec. bachelor's calib. curve: Aware, by group")
legend(8, 0.2, c('fit. whites', 'fit. blacks', 'obs. whites', 'obs. blacks'), 
       lty = c(1, 2, NA, NA), lwd = c(2, 2, NA, NA), pch = c(NA, NA, 0, 1), 
       col = c('black', 'black', 'blue', 'red'))
axis(side = 1, c(1:10))
...
plot(whites$x, whites$y, type = 'l', lty = 1, col = 'red', 
     xlab = "Change in pred. prob. of rec. bachelor's", 
     ylab = "Density", xlim = c(-0.15, 0.15))
...
for (i in 1:length(covar.list)) {
  resid = lsfit(black.orth, orthog_covar[[covar.list[i]]], intercept = FALSE)$residuals
...
title("Rec. bachelor's sampsize: orthog")
legend("topright", c("Default", "20%", "40%", "60%", "80%"), 
       lty = c(1, 1, 1, 1, 1), lwd = c(1, 1, 1, 1, 1), 
       col = c("blue", "red", "green", "orange", "cyan"))
...
# Optimal mtry = 320
set.seed(5000)
RF_orthog = randomForest(orthog_covar[train, ], y[train], ntree = 1500, sampsize = 0.2*trg_sz, mtry = 320)
...
blind_admit[race_covar.tst$white == 1 & raceblind_prob == whiteblind_cutoff] = c(1, 0, 0)
blind_admit.wht = sum(race_covar.tst$white[blind_admit == 1])
blind_admit.blk = sum(race_covar.tst$black[blind_admit == 1])
